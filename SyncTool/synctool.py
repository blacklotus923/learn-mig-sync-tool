...
class File:
    def __init__(self,name,parent,type,size,modified=None,state=2):
        self.name=name
        self.parent=parent
        self.type=type
        self.size=size
    ...
    if path:
        for object in os.listdir(path):
            ...
            if os.path.isdir(fullpath):
                yield File(object,path,'DIR',os.path.getsize(fullpath),os.path.getmtime(fullpath))
                #print('DIRECTORY')
                yield from files(fullpath)
            else:
                yield File(object, path, 'FILE',os.path.getsize(fullpath),os.path.getmtime(fullpath))
...
def dictSave(filename, dict):
    try:
        with open(filename, 'wb') as f:
            cpickle.dump(dict,f, protocol=3)
        return True
    except IOError as e:
...
def filecompare():
    ...
    if hashedfilename(configuration["_fname"]) in configuration["history"]:
        try:
            oldfiles = dictLoad(hashedfilename(configuration["_fname"]))
        except:
            oldfiles = {}
    else:
        oldfiles = {}
...
def setdir():
    ...
    b = True
    while b:
        dir = input("Enter path to recursively search, c to cancel:  ")
        if dir == "c":
            b=False
        elif os.path.isdir(dir):
            dir = os.path.abspath(dir)
            configuration["_cwd"] = dir
            b=False
        else:
            print(dir + " doesn't appear to be a directory or access is denied.")
    return True
...
def scandir():
    ...
    if os.path.isdir(configuration["_cwd"]):
        ...
        print('\nSize of dict in memory is ' + sizeof_fmt(getsizeof(newfiles)))
...
def showlast():
    ...
    if hashedfilename(configuration["_cwd"]) in configuration["history"]:
        try:
            oldfiles = dictLoad(hashedfilename(configuration["_cwd"]))
...
def switchplate(argument):
    ...
    func = switchplate.get(argument, lambda: True)
    return func()
...
try:
    configuration=dictLoad(_configfile)
except IOError as e:
    if e.errno==2:
...
#Exits when a function returns false, e.g. the exit() function.
b=True
while b==True:
    select = menu().lower()
    b = switchplate(select)
    try:
        dictSave(_configfile, configuration)
    except:
        print("Could not save new config.  Check directory permissions on " + str(os.path.abspath(os.getcwd())))
